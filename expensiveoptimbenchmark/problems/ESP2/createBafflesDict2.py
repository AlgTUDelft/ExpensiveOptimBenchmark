...
def write(name,val,conf):
    if val==0:
        ...
        writeFromArray(name, val-2, conf)
...
def writeFromArray(name, val, porousness):
    createBafflesDictFile.write(name+"{type faceZone; zoneName "+name+"; patches{master{name "+name+"_m; type cyclic; neighbourPatch "+name+"_s; patchFields{p{type porousBafflePressure; patchType cyclic; D 0; I "+str(porousness)+"; length 0.03; jump uniform 0;value uniform 0;}}}slave{name "+name+"_s; type cyclic; neighbourPatch "+name+"_m; patchFields{${...master.patchFields}}}}}\n")
