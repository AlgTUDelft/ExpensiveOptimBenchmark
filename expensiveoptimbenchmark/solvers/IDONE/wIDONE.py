...
def optimize_IDONE(problem, max_evals, rand_evals=0, model='advanced', binarize_categorical=False, binarize_int=False, sampling = None, enable_scaling=False, log=None, exploration_prob='normal'):
    d = problem.dims()
    lb = problem.lbs()
    ub = problem.ubs()
    ...
    mon.start()
    solX, solY, model, logfile = IDONE_minimize(f, x0, lb, ub, max_evals, rand_evals=rand_evals, enable_scaling=enable_scaling, model_type=model, sampling = sampling, exploration_prob=prob)
    mon.end()
